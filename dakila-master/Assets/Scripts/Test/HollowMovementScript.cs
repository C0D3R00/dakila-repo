...
public class HollowMovementScript : MonoBehaviour
{
    ...
    void GetInputs()
    {
        ...
        if (/*!Input.GetButton("Jump")*/ !Input.GetKey(KeyCode.Space) && stepsJumped < jumpSteps && stepsJumped > jumpThreshold && pState.jumping)
        {
            StopJumpQuick();
        }
        else if (/*!Input.GetButton("Jump")*/ !Input.GetKey(KeyCode.Space) && stepsJumped < jumpThreshold && pState.jumping)
        {
            StopJumpSlow();
        }
    }
